...
class county(object):
    ...
    def update(self, neighbor_influence): 
        ...
        total_increase = \
        np.rint(self_increase*self_coeff + neighbor_influence*neighbor_coeff)
...
def _get_neighbor_influence(last_increases, neighbors, relation):
    ...
    for fips in all_fips:
        ...
        for n_fips in neighbor_fips:
            ...
            influence += \
            relation[fips][n_fips]*coeff*last_increases[n_fips]
...
def simulate(init_increases, init_reports, neighbors, relation):
    ...
    for _ in range(simulate_years):
        # simulate one year
        neighbor_influence = \
        _get_neighbor_influence(last_increase, neighbors, relation)
        result = dict()
...
if __name__ == '__main__':
    ...
    for substance in substances:
        init_increases, init_reports, \
            neighbors, relation = load_data(substance)
